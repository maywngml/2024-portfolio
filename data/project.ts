...
export const projects: Project[] = [
  {
    ...
    details: [
      ...
      '서버 컴포넌트를 최대한 활용하기 위해 컴포넌트를 세분화하여 코드의 재사용성과 유지보수성을 높였고, Plaiceholder를 사용해 이미지 크기와 블러 이미지를 사전 설정한 이미지 서버 컴포넌트를 개발해 레이아웃 시프트를 최소화하고 UX를 개선했습니다.',
      'Wagmi를 활용해 블록체인 지갑 연결 기능을 구현하고, Next.js API 라우트, MongoDB와 Google APIs를 사용해 DAO 멤버 신청 기능을 개발했습니다. 또한, GSAP와 CSS로 텍스트 슬라이드 인, 페이드 인, 바운스 애니메이션을 구현해 시각적 몰입도를 높였습니다.',
    ],
    achievements: [
      // TODO: 수정
  ...
  // },
  {
    ...
    details: [
      '해당 프로젝트에서는 처음으로 노드 민팅 기능을 구현하면서 거래 안전성에 중점을 두었습니다.',
      '사용자의 화이트리스트 대상 여부와 블록체인 지갑 네트워크 설정을 각 단계에서 점검해 거래 오류 방지에 신경 썼습니다.',
      '스마트 컨트랙트와의 상호작용에서 발생하는 트랜잭션 순서를 철저히 관리해 거래 안전성을 높였으며, 다양한 에러 시나리오를 고려해 공통 에러 핸들링 로직을 구축하고, 사용자에게 명확한 피드백을 제공했습니다.',
      '또한, API 스웨거 문서화 작업을 통해 협업 효율성을 높이고, 추천인 코드 소유자의 블록체인 지갑 주소 조회 API를 추가 개발했습니다.',
    ],
    achievements: [
      // TODO: 좀 더 구체적으로 수정
  ...
  },
  {
    ...
    details: [
      'APE UNIVERSE는 다양한 애니메이션을 통해 인터랙티브한 웹을 개발하는 데 중점을 두었습니다.',
      'GSAP를 활용해 버튼 클릭 모션, 로켓 발사, 행성 이동 등의 애니메이션을 구현하여 웹에 생동감을 부여했으며, 애니메이션은 반응형으로 개발해 모바일과 pc 모두에서 일관된 사용자 경험을 제공했습니다.',
      '또한, Wagmi를 사용해 블록체인 지갑 연결과 네트워크 변경 기능을 구현하고, Next.js API 라우트와 MongoDB, Google APIs를 활용해 프로젝트 신청 기능을 개발했습니다.',
    ],
    achievements: [
  ...
  },
  {
    ...
    details: [
      '처음으로 마크업부터 퍼블리싱, 기능 구현까지 모든 과정을 직접 경험한 프로젝트였습니다.',
      '스크롤 기반 이미지 시퀀스 애니메이션을 구현하고, ResizeObserver를 사용해 브라우저 화면 리사이징에 대응하여 모든 사용자에게 일관된 경험을 제공했습니다.',
      '또한, Next.js API 라우트와 MongoDB, Google APIs를 활용해 멤버십 신청 기능을 구현했습니다.',
      '잦은 기획 변경에도 Git을 활용한 체계적인 버전 관리로 프로젝트를 성공적으로 진행했습니다.',
    ],
    achievements: [
  ...
  },
  {
    ...
    details: [
      ...
      'GSAP를 활용하여 스크롤 위치에 따라 요소가 회전하며 움직이는 스크롤 애니메이션과 콘텐츠가 컨베이어 벨트처럼 한쪽으로 흘러가는 무한 스크롤 애니메이션을 개발했습니다. 이 외에도 역동적이고 다채로운 사이트를 만들기 위해 다양한 애니메이션을 적용했습니다.',
    ],
    achievements: [
