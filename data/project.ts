...
export const projects: Project[] = [
  ...
  // },
  {
    ...
    overview:
      '블록체인 기술을 활용하여 가상 자산 투자를 돕는 플랫폼의 공식 웹 서비스를 개발했습니다.',
    details: [
      '해당 프로젝트에서는 NFT 구매 과정을 간소화하여 사용자의 조작을 최소화하고 직관적인 구맥 경험을 설계하는 것에 중점을 두었습니다.',
      ...
      '이 외에도 API 수정과 신규 기능 개발을 지원해 마감 기한을 2일 단축하며 개발 생산성을 향상했습니다.',
      '추가로, 각 API와 블록체인 지갑 연결, 트랜잭션 연동 과정에서 반환되는 에러 객체가 서로 달라 일관된 에러처리가 어려웠습니다. 이를 해결하기 위해 공통 에러 객체를 생성하고, 블록체인 지갑 연결과 트랜잭션 연동 시 catch절에서는 각 상황에 맞는 메시지를 담은 에러 객체를 반환하도록 했습니다. 또한, API에서도 클라이언트에 전달하는 에러 응답을 통일해 사용자에게 오류 사항과 대응 방안을 명확히 제공할 수 있도록 했습니다.',
    ],
    achievements: [
  ...
  // },
  {
    ...
    details: [
      ...
      '또한, SEO 최적화를 통해 Lighthouse 검사 결과 모든 페이지의 SEO 점수를 100점으로 달성했습니다.',
      ...
      '배포 이후, 초기 진입 시 표시되는 동영상이 재생되지 않는 문제가 발생했으나, 빠르게 원인을 분석하여 CDN 오류임을 파악했었습니다. 이후 Youtube 영상으로 대체하여 원활한 서비스가 제공이 가능하도록 대처했습니다.',
    ],
  ...
  },
  {
    ...
    // teamSize: '프론트엔드 1명',
    overview:
      '복합 예술 문화단체인 ARTIVIST의 공식 웹 사이트를 개발하였습니다.',
    details: [
      '이 프로젝트는 마크업부터 기능 구현까지 모든 과정을 직접 경험한 첫 번째 프로젝트였습니다. PC와 모바일 환경에서 반응형 웹을 구현하여 다양한 기기에서 일관된 사용자 경험을 제공했으며, 스크롤 기반 이미지 시퀀스 애니메이션을 구현해 몰입감 있는 사용자 경험을 제공했습니다.',
      '초기에는 SSR을 적용했지만, 개발 서버 배포 후 페이지 로드 속도가 4초나 걸리는 문제를 발견했습니다. 이에 대한 원인을 분석하면서 렌더링 방식에 대해 더 깊이 학습하게 되었고, SSG로 전환하여 로드 시간을 1초로 단축할 수 있었습니다.',
  ...
  // },
  {
    ...
    details: [
      ...
      '잘못된 이메일 주소 입력을 방지하기 위해 JWT 토큰과 Nodemailer를 활용한 이메일 인증 기능을 구현했습니다. 또한, 산타의 답장은 OpenAI API를 통해 자동으로 생성되며, 따뜻한 위로와 공감을 담기 위해 칭찬, 공감, 격려를 중심으로 프롬프트를 작성했습니다.',
    ...
    achievements: ['5일동안 방문자 수 515명, 참여자 수 63명 달성'],
    image: {
